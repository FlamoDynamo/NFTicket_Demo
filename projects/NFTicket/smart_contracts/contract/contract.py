...
def approval_program():
    ...
    handle_create_event = Seq([
        ...
        Log(Concat(Bytes("New event count: "), Itob(App.globalGet(EVENT_COUNT)))),
    ...
    # Mã xử lý add_attendant
    handle_add_attendant = Seq([
        Log(Bytes("Handling add_attendant")),
        
        # Kiểm tra event_id hợp lệ và còn vé
        Assert(App.globalGet(Concat(EVENT_TICKET_COUNT_PREFIX, int_to_str(Btoi(Txn.application_args[1])))) > Int(0)),
        Log(Bytes("Event has available tickets")),
        
        # Kiểm tra người tham gia chưa đăng ký vào sự kiện này
        App.localGetEx(Txn.sender(), App.id(), Concat(EVENT_ATTENDANTS_PREFIX, int_to_str(Btoi(Txn.application_args[1])))),
        Assert(Not(App.localGet(Txn.sender(), Concat(EVENT_ATTENDANTS_PREFIX, int_to_str(Btoi(Txn.application_args[1])))))),
        Log(Bytes("Attendant is not already registered")),
        
        # Cập nhật trạng thái tham gia của người dùng
        App.localPut(Txn.sender(), Concat(EVENT_ATTENDANTS_PREFIX, int_to_str(Btoi(Txn.application_args[1]))), Int(1)),
        Log(Bytes("Attendant registered")),

        # Giảm số lượng vé còn lại và tăng số lượng vé đã phát hành
        App.globalPut(
        ...
        Log(Bytes("Ticket count updated")),
        ...
        Log(Bytes("Ticket issuance updated")),
